...
public IActionResult Demote(string id)
{
    ...
    var adminCount = _dbContext.UserRoles
    ...
    if (adminCount <= 1)
    {
        ...
        return BadRequest(new { message = "You cannot demote the last admin. There must always be at least one admin." });
    }
    ...
    public IActionResult DeactivateAccount(int id)
    {
        ...
        var isAdmin = _dbContext.UserRoles.Any(ur => ur.UserId == userProfile.IdentityUserId && ur.RoleId == _dbContext.Roles.SingleOrDefault(r => r.Name == "Admin").Id);

        // If the user is an admin, we need to check the number of active admins
        if (isAdmin)
        {
            var activeAdminCount = _dbContext.UserProfiles.Count(up => up.IsActive && _dbContext.UserRoles.Any(ur => ur.UserId == up.IdentityUserId && ur.RoleId == _dbContext.Roles.SingleOrDefault(r => r.Name == "Admin").Id));

            // Prevent deactivating the last active admin
            if (activeAdminCount <= 1 && userProfile.IsActive)
            {
                return BadRequest("You cannot deactivate the last active admin. There must always be at least one active admin.");
            }
        }
